...
import 'package:pcrypt/util/shared_password_model.dart';
...
class JavaScripts {
  ...
  Future<List<String>> loadScriptFiles() async {
    return [
      ...
      await rootBundle
          .loadString('js/lib/srp6a/thinbus-srp6a-sha256-versioned.js'),
  ...
  Future<LoginKeys> loginStep1Part1(Login1Result data, String password) async {
    return LoginKeys.fromJson(_smartDecode(jsonDecode(await _evaluate(
  ...
  Future<LoginCredentials> loginStep1Part2(
      Login1Result data, LoginKeys keys) async {
    return LoginCredentials.fromJson(_smartDecode(jsonDecode(await _evaluate(
  ...
  Future<AccountData> createAccount(String email, String password) async {
    final Map<String, dynamic> result = _smartDecode(jsonDecode(await _evaluate(
  ...
  Future<PCryptKey> decodePrivateKey(PCryptKey key, String aes) async {
    return PCryptKey(
      ...
      ecdh: EllipticCurve(
        ...
        data: _smartDecode(await _evaluate(
  ...
  Future<String> getSharedKey(
      PCryptKey? publicKey, PCryptKey privateKey) async {
    return _smartDecode(await _evaluate(
  ...
  Future<Map<int, Map<int, String>>> getTeamKeys(
    ...
    //Prepare members
    final membersData = jsonEncode(members.map((m) {
      final map = m.toJson();
    ...
    final Map<String, dynamic> result = _smartDecode(await _evaluate(
  ...
  Future<Encrypted> markNewShares(
    ...
    final evaluateResult =
        await _evaluate('markNewShares($sharesData, $oldShareIds);');
  ...
  Future<SharedPasswordModel> decodeSharedPasswords(
    ...
    // Prepare shares
    final userSharesData =
        jsonEncode(shares.where((s) => s.type == TeamShareType.user).map((s) {
      final map = s.toJson();
      map['data'] = jsonEncode(map['data']);
      return map;
    }).toList());
    final teamSharesData =
        jsonEncode(shares.where((s) => s.type == TeamShareType.team).map((s) {
      final map = s.toJson();
      map['data'] = jsonEncode(map['data']);
      return map;
    }).toList());
    ...
    final teamKeysData = jsonEncode(
      ...
      return MapEntry<String, dynamic>(teamId.toString(),
    ...
    final List<Password> userPasswords = decryptedUser
        .map<List<Password>>((dynamic object) {
          ...
          final List<Password> value =
              List<Password>.from(object['data'].map((dynamic e) {
    ...
    final List<Password> teamPasswords = decryptedTeam
        .map<List<Password>>((dynamic teamValue) {
          ...
          return List<Password>.from(teamValue['data'].map((dynamic e) {
            final password = Password.fromJson(e);
    ...
    return SharedPasswordModel(
        userSharedPasswords: userPasswords, teamSharedPasswords: teamPasswords);
  }
  ...
  Future<Map<int, List<int>>> findShareChanges(
    ...
    final encodedOld =
        jsonEncode(oldShares.map<String, dynamic>((int id, List<int> teamsMap) {
      return MapEntry<String, dynamic>(id.toString(), teamsMap);
    }));
    final encodedNew =
        jsonEncode(newShares.map<String, dynamic>((int id, List<int> teamsMap) {
      return MapEntry<String, dynamic>(id.toString(), teamsMap);
    }));
    final result =
        await _evaluate('findShareChanges($encodedOld, $encodedNew);');
    final Map<String, dynamic> decodedResult = _smartDecode(jsonDecode(result));
    return Map<int, List<int>>.from(decodedResult.map<int, List<int>>(
  ...
  Future<Share1Result> sharePasswordsPart1(
    ...
    //Execute method
    return Share1Result.fromJson(_smartDecode(jsonDecode(await _evaluate(
  ...
  Future<Share2ResultV2> sharePasswordsPart2(
    ...
    final Map<String, dynamic> result =
        _smartDecode(jsonDecode(evaluateResult));
  ...
  Future<String> hashMap(Map<dynamic, Encrypted> data) async {
    final String dataString =
        jsonEncode(jsonEncode(data.map<String, String>((dynamic k, v) {
