...
class WebProvider {
  ...
  // String get _endpoint => _currentServer + '/lib/pcrypt.php';

  String get _strings => _currentServer + '/lib/lang/languagedb.php';

  // String get _strings => '/lib/lang/languagedb.php';

  Dio? _dioClient;

  Dio? getClient({bool enforceDefaultDomain = false}) {
    // if (_dioClient == null || enforceDefaultDomain == true) {
  ...
  Future<WebResult<T>> _genericWebRequest<T>(
      String method, String? session, String bodyData,
      {T Function(dynamic)? resultTransformer,
      bool enforceDefaultDomain = false}) async {
    try {
      final String body =
          '{"method": "$method", "session": "$session", "data": $bodyData, "id": "0" }';
      final res = await getClient(enforceDefaultDomain: enforceDefaultDomain)
      ...
      if (res != null && res.statusCode == 200) {
        ...
        return WebResult.success(resultTransformer != null
  ...
  Future<WebResult<Login2Result>> loginStep2(
      String email, String srpA, String srpM1, String? pin) async {
    return _genericWebRequest(
      'login2',
      null,
      ...
      resultTransformer: (dynamic result) {
        ...
        result['privatekey']['ecdh']['data'] =
            jsonDecode(result['privatekey']['ecdh']['data']);
        result['privatekey']['ecdsa']['data'] =
            jsonDecode(result['privatekey']['ecdsa']['data']);
  ...
  Future<WebResult<bool>> globalEmergencyStatus(String session) async {
    return _genericWebRequest(
      ...
      '{}',
      resultTransformer: (dynamic result) {
        print('globalEmergencyStatus result');
  ...
  Future<WebResult<int>> signUp(AccountData data, Language language) async {
    return _genericWebRequest('create', null,
  ...
  Future<WebResult<Encrypted?>> getEncryptedData(
      String session, DataName dataName) async {
  ...
  Future<WebResult<bool>> setEncryptedData(
      String session, DataName dataName, Encrypted? data, String hash) async {
  ...
  Future<WebResult<Encrypted>> getBinaryData(
      String session, String fileId) async {
    return _genericWebRequest(
      'getbinary',
      session,
      '{"dataname": "$fileId"}',
  ...
  Future<WebResult<Map<String, Encrypted>>> getBinaryDataList(
      String session, List<String> fileIds) async {
    return _genericWebRequest(
      'getbinary',
      session,
      ...
      resultTransformer: (dynamic result) {
        ...
        return result.map<String, Encrypted>((dynamic k, dynamic v) =>
  ...
  Future<WebResult<Encrypted>> getUserSharedBinaryData(
      String session, String fileId, int fromId) async {
    return _genericWebRequest(
      'teamgetbinary',
      session,
  ...
  Future<WebResult<Encrypted>> getTeamSharedBinaryData(
      String session, String fileId, int teamId) async {
    return _genericWebRequest(
      'teamgetteambinary',
      session,
  ...
  Future<WebResult<bool>> setBinaryData(
      String session, String fileId, Encrypted data) async {
    final String dataString = jsonEncode(data.toJson());
    return _genericWebRequest('setbinary', session,
  ...
  Future<WebResult<List<String>>> loadFaviconData(
      String session, List<String> urls) async {
  ...
  Future<WebResult<bool>> makeErrorLog(
      String? session, String errorText) async {
    final requestBody = {
      'label': 'fluttererror',
      'text': errorText,
      'shown': false,
      ...
      resultTransformer: (dynamic data) {
        ...
        return Map<int, Uint8List>.from(
          ...
          return MapEntry<int, Uint8List>(
              k == 'default' ? -1 : int.parse(k), base64Decode(v));
        }));
      },
    );
    return avatars;
  ...
  Future<WebResult<void>> setMemberShares(
      String session, String? dataString, String hash) async {
    return _genericWebRequest(
      'teamsetshares',
      session,
  ...
  Future<WebResult<void>> setMemberBinary(
      ...
      Map<int, List<String>> noDelSource) async {
    // Prepare arrays
    ...
    final dataStrings =
        jsonEncode(shareFiles.map((f) => jsonEncode(f.data)).toList());
  ...
  Future<WebResult<void>> setTeamBinary(
    ...
    // Prepare arrays
    final teamId =
        jsonEncode(shareFiles.map((f) => f.teamId.toString()).toList());
  ...
  Future<WebResult<TeamMemberIdPair>> createTeam(
      String session, String name, String contact, String email) async {
    return _genericWebRequest(
      'teamcreate',
      session,
  ...
  Future<WebResult<void>> addNewTeamMembers(
      String session, List<TeamMember> members, Language language) {
  ...
  ///Register new user
  Future<WebResult<Map<int, int>>> sendMessage(String session, Encrypted from,
      Map<int, Encrypted> to, Language language) async {
    final String fromData = jsonEncode(jsonEncode(from.toJson()));
    final String toData =
        jsonEncode(to.map<String, String>((int k, Encrypted v) {
  ...
  Future<WebResult<void>> deleteMessages(
      String session, List<Message> messages) async {
    if (messages.isNotEmpty) {
      final ids = messages.map((m) => m.mailId).join(',');
      return _genericWebRequest(
          'teamremovemail', session, '{"mailid": [$ids]}');
    }
  ...
  Future<WebResult<void>> updateUserInfo(String session, User user) async {
    final avatarString =
        user.avatar.isNotEmpty ? base64Encode(user.avatar) : '';
    return _genericWebRequest(
      'updateinfo',
      session,
  ...
  String locationViewerUrl(
      String auth, Iterable<Location> locations, Location? currentLocation) {
  ...
  Future<WebResult<GeocodeResult>> locationFromAddress(
      String auth, String address) async {
    // final url = Uri.parse('$_currentServer/lib/other/geocode.php?auth=$auth');
    try {
      final res = await getClient()
          ?.post<dynamic>('/lib/other/geocode.php?auth=$auth', data: address);
      // final response = await http.post(url, body: address);
  ...
  Future<WebResult<T>> _stringsWebRequest<T>(
    ...
    try {
      ...
      final res =
          await stringDio.post<dynamic>(_strings, data: <String, String>{
        'method': method,
  ...
  Future<WebResult<List<Language>>> getLocalizationLanguages() async {
    return _stringsWebRequest(
      'languages',
      resultTransformer: (data) => List<Language>.from(data
          .map(
            (k, dynamic v) => MapEntry<String, Language>(
                k, Language(code: k, name: v['name'])),
          )
          .values),
    );
  }
