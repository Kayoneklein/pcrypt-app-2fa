...
class LocalDBService {
  ...
  Future<int?> addPassword(Password pass) async {
    try {
      ...
      final password = await _localDb?.rawQuery(
          '''SELECT * FROM ${LocalDBTable.password} WHERE id=?''', [pass.id]);
      int? passId;
  ...
  Future<int?> addGroup(Group grp) async {
    try {
      ...
      final group = await _localDb?.rawQuery(
          '''SELECT * FROM ${LocalDBTable.group} WHERE id=?''', [grp.id]);
      int? gp;
  ...
  Future<int?> _addGroup(Group grp) async {
    try {
      ...
      final group =
          await _localDb?.rawInsert('''INSERT INTO ${LocalDBTable.group} 
      (id, data) VALUES(?, ?)''', [grp.id, encrypted]);
  ...
  Future<int?> addTeam(Team team) async {
    try {
      await _initDB();
      final tms = await _localDb?.rawQuery('''SELECT * FROM ${LocalDBTable.team}
      WHERE id=?''', [team.id]);
  ...
  Future<int?> _addTeam(Team team) async {
    try {
      ...
      final tm = await _localDb?.rawInsert('''INSERT INTO ${LocalDBTable.team} (
      id, data) VALUES (?, ?)''', [team.id, encrypted]);
  ...
  Future<int?> _updateTeam(Team team) async {
    try {
      ...
      final tm = await _localDb?.rawUpdate('''UPDATE ${LocalDBTable.team} SET
       data=? WHERE id=?''', [encrypted, team.id]);
  ...
  Future<int?> addTeamMembers(TeamMember member) async {
    try {
      await _initDB();
      final mem =
          await _localDb?.rawQuery('''SELECT * FROM ${LocalDBTable.teamMembers}
          WHERE userid=? AND teamid=?''', [member.userId, member.teamId]);
      int? mems;
  ...
  Future<int?> _updateTeamMembers(TeamMember member) async {
    try {
      ...
      final String encrypted = Encryption().encrypt(data);
      final mem =
          await _localDb?.rawUpdate('''UPDATE ${LocalDBTable.teamMembers} SET
  ...
  Future<int?> addRemoteConfig(RemoteConfiguration config) async {
    try {
      ...
      final conf =
          await _localDb?.rawInsert('''INSERT INTO ${LocalDBTable.remoteConfig}
  ...
  Future<int?> addEncrypted(Encrypted encrypt, DataName name) async {
    try {
      await _initDB();
      final query = await _localDb?.rawQuery('''SELECT * FROM 
      ${LocalDBTable.encrypted} WHERE name=?''', [name.name]);
      if (query?.isNotEmpty == true) {
  ...
  Future<int?> _addEncrypted(Encrypted encrypt, DataName name) async {
    try {
      ...
      final enc = await _localDb?.rawInsert('''INSERT INTO 
  ...
  Future<int?> _updateTeamShare(TeamShare share) async {
    try {
      ...
      final tm =
          await _localDb?.rawUpdate('''UPDATE ${LocalDBTable.teamShare} SET
  ...
  Future<int?> addFavicons(List<String> icons) async {
    try {
      ...
      for (final icon in icons) {
        final count = await _localDb
            ?.rawQuery('''SELECT data FROM ${LocalDBTable.passwordFavicons} 
            WHERE data=?''', [icon]);
