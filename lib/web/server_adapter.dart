...
import 'package:pcrypt/model/geocode_result.dart';
import 'package:pcrypt/model/global_message.dart';
import 'package:pcrypt/model/group.dart';
import 'package:pcrypt/model/location.dart';
import 'package:pcrypt/model/login.dart';
import 'package:pcrypt/model/login_result.dart';
...
import 'package:pcrypt/model/share_result.dart';
...
import 'package:pcrypt/util/readonly_service.dart';
...
class ServerAdapter {
  ...
  bool checkResultForErrors<T>(
      WebResult<T> result, Function(AdapterError)? onError) {
    if (result.hasError) {
      //Raf
  ...
  Future<void> login({
    ...
    try {
      ...
      final LoginKeys keys =
          await _scripts.loginStep1Part1(result1.data!, password);

      final LoginCredentials credentials =
          await _scripts.loginStep1Part2(result1.data!, keys);
      //Step 2
      final WebResult<Login2Result> result2 =
          await _web.loginStep2(email, credentials.A, credentials.M1, pin);
      ...
      final privateKey =
          await _scripts.decodePrivateKey(result2.data!.privateKey, keys.aes);
      ...
      await _storage.save(
          _StorageKey.privateKey, jsonEncode(privateKey.toJson()));
  ...
  Future<void> signUp({
    ...
    try {
      ...
      final WebResult<int> result =
          await _web.signUp(data, Localization.get.currentLanguage);
  ...
  Future<void> loadPasswords({
    ...
    try {
      ...
      if (await Preferences().readonlyMode == true) {
        ...
        final WebResult<Encrypted?> passwordsResult =
            await _web.getEncryptedData(session, DataName.passwords);
        ...
        if (passwordsResult.data != null) {
          final dynamic passwordsDecrypted =
              await _scripts.decodeData(passwordsResult.data!, aes);
      ...
      final iconResult = await _web.loadFaviconData(
          session, result.map((p) => p.url).toList());
  ...
  Future<void> loadGroups({
    ...
    try {
      ...
      final WebResult<Encrypted?> groupsResult =
          await _web.getEncryptedData(session, DataName.groups);
      ...
      if (await Preferences().readonlyMode == true) {
        ...
        if (groupsResult.data != null) {
          final dynamic groupsDecrypted =
              await _scripts.decodeData(groupsResult.data!, aes);
    ...
    try {
      ...
      if (passwords.isNotEmpty) {
        ...
        data = await _scripts.encodeData(
            passwords.map((p) => p.toJson()).toList(), aes);
      ...
      final String session = await _storage.load(_StorageKey.session);
      final result =
          await _web.setEncryptedData(session, DataName.passwords, data, hash);
      if (checkResultForErrors(result, onError)) {
        return;
      }
      onSuccess?.call();
    ...
    try {
      ...
      if (groups.isNotEmpty) {
        try {
          ...
          print('error from saving groups to local db in server adapter');
        }
        final String aes = await _storage.load(_StorageKey.aes);
        data = await _scripts.encodeData(
            groups.map((g) => g.toJson()).toList(), aes);
        hash = await _scripts.hash(data);
      } else {
        data = null;
        hash = await _scripts.emptyHash();
      ...
      final String session = await _storage.load(_StorageKey.session);
      final result =
          await _web.setEncryptedData(session, DataName.groups, data, hash);
      if (checkResultForErrors(result, onError)) {
        return;
      }
      onSuccess?.call();
  ...
  Future<void> loadFile({
    ...
    try {
      final String session = await _storage.load(_StorageKey.session);
      final String aes = await _storage.load(_StorageKey.aes);
      final WebResult<Encrypted> result =
          await _web.getBinaryData(session, fileId);
      if (checkResultForErrors(result, onError)) {
        return;
      }
  ...
  Future<void> loadUserSharedFile({
    ...
    try {
      ...
      final String session = await _storage.load(_StorageKey.session);
      final WebResult<Encrypted> result =
          await _web.getUserSharedBinaryData(session, fileId, ownerId);
      if (checkResultForErrors(result, onError)) {
        return;
      }
      // Decrypt loaded data
      ...
      final String sharedKey =
          await _scripts.getSharedKey(publicKey, privateKey);
      final decodedBinary =
          await _scripts.decodeBinaryData(result.data!, sharedKey);
  ...
  Future<void> loadTeamSharedFile({
    ...
    try {
      ...
      final String session = await _storage.load(_StorageKey.session);
      final WebResult<Encrypted> result =
          await _web.getTeamSharedBinaryData(session, fileId, teamId);
      if (checkResultForErrors(result, onError)) {
        return;
      }
      // Decrypt loaded data
      final decodedBinary =
          await _scripts.decodeBinaryData(result.data!, teamKey);
      onSuccess?.call(decodedBinary);
    } catch (error) {
      onError?.call(AdapterError.fromException(error));
    }
          ...
          onSuccess: (config) async {
            if (config.disableTeams) {
              ...
              if (checkSharingOptions) {
                ...
                members =
                    members.where((m) => teamIds.contains(m.teamId)).toList();
              }
              ...
              onSuccess: (config) async {
                if (config.disableTeams) {
                  ...
                  if (shares.isNotEmpty) {
                    ...
                    final PCryptKey privateKey = PCryptKey.fromJson(jsonDecode(
                        await _storage.load(_StorageKey.privateKey)));
                    final String aes = await _storage.load(_StorageKey.aes);
                    final currentId =
                        int.parse(await _storage.load(_StorageKey.userId));
                    // Prepare teams and members arrays
                    final Map<int, Team?> teams =
                        teamsList.asMap().map((k, v) => MapEntry(v.id, v));
                    final List<int> teamIds =
                        teamsList.map((t) => t.id).toList();
                    final List<TeamMember> members =
                        membersList.where((TeamMember m) {
                      return m.userId != -1 &&
                          m.teamId != -1 &&
                          m.publicKey != null &&
                    ...
                    final teamKeys = await _scripts.getTeamKeys(
                        teamIds, members, currentId, privateKey, aes);
                    // Decrypt passwords
                    ...
                    final List<String> currentPasswordIdsHashList =
                      ...
                      final hashPassId = await _scripts.stringHash(hashData);
                    ...
                    if (sharedPrefPasswordIdsHashList != null) {
                      final Map<String, List<Password>> hashAndPassword =
                          groupBy(
                              currentPasswords, (Password obj) => obj.hashId!);
                      final sharedPrefPasswordSet =
                          Set.of(sharedPrefPasswordIdsHashList);
                      final currentPasswordIdsSet =
                          Set.of(currentPasswordIdsHashList);
                      ...
                      final diffsAdd = currentPasswordIdsSet
                          .difference(sharedPrefPasswordSet);
                    ...
                    // Save to SharedPreference when first login
                      ...
                      await _settings.saveStringList(Settings.kPasswordsPrefKey,
                    ...
                    if (await Preferences().readonlyMode != true) {
                      final icons = await _web.loadFaviconData(
                          session, allPasswords.map((p) => p.url).toList());
                    ...
                    for (int i = 0; i < allPasswords.length; i++) {
                      ...
                      for (TeamMember member in members.where((m) =>
                          m.userId == (password.creator?.userId ?? -2) &&
                          password.shareTeamIds.contains(m.teamId))) {
                        final currentMember = members.firstWhere((m) =>
                            m.userId == currentId && m.teamId == member.teamId);
                        password.isHidden |= currentMember.teamHidePassword ||
      ...
      if (resultData.isNotEmpty) {
        ...
        if (uidMap != null) {
          for (final entry in uidMap.entries) {
            ...
            final publicKey =
                PCryptKey.fromJson(publicKeyJson ?? <String, dynamic>{});
            final String sharedKey =
                await _scripts.getSharedKey(publicKey, privateKey);
            final encryptedPassword =
                await _scripts.encodeData(password, sharedKey);
            dataForSetEmergency[entry.key] =
                jsonEncode(encryptedPassword.toJson());
          }
        }
      }
      ...
      onSuccess: (allTeams, allMembers) async {
        try {
          ...
          final List<Password> copiedPasswords = myPasswords.map<Password>((p) {
            return p.copyWith(
              shares: Map.from(p.shares.map<int, List<int>>(
          ...
          for (Password password in copiedPasswords) {
            final needsToBeShared =
                passwordsToShare.any((p) => p.tempId == password.tempId);
            if (needsToBeShared) {
              final oldShares = Map<int, List<int>>.from(password.shares
                  .map<int, List<int>>(
                      (int k, List<int> v) => MapEntry(k, List.from(v))));
              ...
              for (TeamMember member in toMembers) {
                if (copiedPassword.shares.containsKey(member.userId)) {
                  if (copiedPassword.shares[member.userId]
                          ?.contains(member.teamId) ==
                      false) {
                    copiedPassword.shares[member.userId]!.add(member.teamId);
                  }
                } else {
                  copiedPassword.shares
                      .putIfAbsent(member.userId, () => [member.teamId]);
                }
              }

              final newShares = copiedPassword.shares;
              final findShareChanges =
                  await _scripts.findShareChanges(oldShares, newShares);
          ...
          final Share1Result share1 = await _scripts.sharePasswordsPart1(
              toShare, allMembers, serverFileData, currentId, fileChanges);
          //Load binary data (for new files)
          final newBinaryResult =
              await _web.getBinaryDataList(session, share1.newFileIds);
          ...
          if (teamIds.isNotEmpty) {
            ...
            teamKeys = await _scripts.getTeamKeys(
                teamIds, filteredMembers, currentId, privateKey, aes);
          }
          ...
          for (var fs in share2.newUserFiles ?? <UserFileShare>[]) {
            fs.data = await _scripts.encodeBinaryData(
                binaryDataDecrypted[fs.fileId] ?? Uint8List(0), fs.shareKey);
          }
          ...
          if (share2.noDelTeamSource != null) {
            ...
            final shareTeamResult = await _web.setTeamShares(
                session, share2.teamDataString, share2.teamHash);
          ...
          final shareMemberResult = await _web.setMemberShares(
              session, share2.userDataString, share2.userHash);
          ...
          if (share2.newUserFiles != null && share2.newUserFiles!.isNotEmpty) {
            ...
            final shareBinaryMembersResult = await _web.setMemberBinary(
                session, share2.newUserFiles!, share2.noDelUserSource!);
          ...
          if (share2.newTeamFiles != null && share2.newTeamFiles!.isNotEmpty) {
            ...
            final shareBinaryResult = await _web.setTeamBinary(
                session, share2.newTeamFiles!, share2.noDelTeamSource!);
        ...
        try {
          ...
          for (Password password in copiedPasswords) {
            ...
            if (needsToBeShared) {
              ...
              final findShareChanges =
                  await _scripts.findShareChanges({}, newShares);
    ...
    await loadTeamsAndMembers(
      onSuccess: (allTeams, allMembers) async {
        try {
          ...
          final List<Password> copiedPasswords =
              allPasswords.map<Password>((p) {
            return p.copyWith(
              shares: Map.from(p.shares.map<int, List<int>>(
                  (int k, List<int> v) => MapEntry(k, List.from(v)))),
              shareChanges: Map.from(
          ...
          for (Password password in copiedPasswords) {
            final needsToBeShared =
                passwordsToDelete.any((p) => p.tempId == password.tempId);
            if (needsToBeShared) {
              final oldShares = Map<int, List<int>>.from(password.shares
                  .map<int, List<int>>(
                      (int k, List<int> v) => MapEntry(k, List.from(v))));
  ...
  Future<void> processNewTeamMembers({
    ...
    try {
      ...
      final addResult = await _web.addNewTeamMembers(
          session, preparedMembers, Localization.get.currentLanguage);
      ...
      final mergedMembers = preparedMembers
          .map<TeamMember>((m) {
            final findings = getResult.data!
                .where((pm) => pm.email == m.email && pm.teamId == m.teamId);
        ...
        try {
          ...
          for (final MessageEncrypted message
            ...
            if (member != null && member.publicKey != null) {
              try {
                final sharedKey =
                    await _scripts.getSharedKey(member.publicKey!, privateKey);
                inbox.add(Message.fromJson(
  ...
  Future<void> sendMessage({
    ...
    await loadTeamsAndMembers(
      onSuccess: (teamsList, membersList) async {
        try {
          final currentId = int.parse(await _storage.load(_StorageKey.userId));
          final membersToShare =
              groupBy(membersList, (TeamMember e) => e.userId);
          ...
          final membersFromTeams =
              groupBy(teamMembersToShare, (TeamMember e) => e.userId).values;
          final teamsAndMembers =
              [...allMembersToShare, ...membersFromTeams].flattened.toSet();
          final sharesFrom =
              groupBy(teamsAndMembers, (TeamMember e) => e.userId)
          ...
          final Encrypted fromData =
              await _scripts.encodeData(messageToSendFrom.toJson(), aes);
          ...
          for (TeamMember member in teamsAndMembers) {
            if (member.publicKey != null) {
              ...
              final result = await _scripts.encodeData(
                  messageToSendTo.toJson(), sharedKey);
          ...
          final sendResult = await _web.sendMessage(
              session, fromData, toData, Localization.get.currentLanguage);
  ...
  Future<void> getUserUpdates({
    ...
    try {
      ...
      final _error = AdapterError.fromException(error);
      onError?.call(_error);
    }
  }
  ...
  Future<Location?> getLocationFromAddress(String address) async {
    try {
      final auth = await _storage.load(_StorageKey.auth);
      final WebResult<GeocodeResult> result =
          await _web.locationFromAddress(auth, address);
      if (result.data != null) {
        ...
        switch (geometry.locationType) {
          ...
          case GeocodeLocationType.approximate:
            final lat1 = geometry.boundsNorthEast?.latitude ??
                geometry.viewportNorthEast.latitude;
            final lng1 = geometry.boundsNorthEast?.longitude ??
                geometry.viewportNorthEast.longitude;
            final lat2 = geometry.boundsSouthWest?.latitude ??
                geometry.viewportSouthWest.latitude;
            final lng2 = geometry.boundsSouthWest?.longitude ??
  ...
  Future<void> getRemoteConfig({
    ...
    if (rawCachedConfig.isNotEmpty) {
      try {
        Future.delayed(Duration.zero, () {
          final cachedConfig =
              RemoteConfiguration.fromJson(jsonDecode(rawCachedConfig));
    ...
    if (checkResultForErrors(configResults, onError)) {
      ...
      _storage.save(
          _StorageKey.remoteConfig, jsonEncode(configResults.data!.toJson()));
    }
  ...
  Future<WebResult> removeAccount(
    ...
    try {
      ...
      if (kDebugMode) {
        print('error from removeAccount() in server adapter: $err');
      }
