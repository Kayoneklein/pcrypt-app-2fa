  ...
  AccountInfoBloc() : super(AccountInfoState.initial()) {
    ...
    on<AccountInfoEvent>((event, emit){
      ...
      if (event is AccountInfoSaved) {
        emit(AccountInfoSavedState.from(state.copyWith(isLoading: false), isSuccess: event.isSuccess));
      }
      //Discard changes
      ...
      if (event is ConnectionErrorEvent) {
        emit(ConnectionErrorState.from(state, errorMessage: event.errorMessage));
      }
