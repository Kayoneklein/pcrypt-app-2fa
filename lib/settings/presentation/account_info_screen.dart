...
import 'package:pcrypt/settings/bloc/account_info.dart';
...
class _AccountInfoFormState extends State<AccountInfoForm> {
      ...
      child: BlocBuilder<AccountInfoBloc, AccountInfoState>(
        ...
        if (_newPasswordController.text != state.newPassword) {
          _newPasswordController.value =
              _newPasswordController.value.copyWith(text: state.newPassword);
        }
  ...
  Future<void> _stateChangeListener(
      BuildContext context, AccountInfoState state) async {
    ...
    if (state is AccountInfoSavedState) {
      if (state.isSuccess) {
        ...
        ScaffoldMessenger.of(context)
          ...
          ..showSnackBar(
            SnackBar(
              content: Text(Strings.accountInfoSaveError),
            ),
          );
      }
    }
