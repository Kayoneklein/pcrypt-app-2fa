...
class NotificationsBloc extends Bloc<NotificationsEvent, NotificationsState> {
  NotificationsBloc() : super(NotificationsState.initial()) {
    ...
    on<NotificationsEvent>((event, emit) {
      ...
      if (event is GlobalMessageCountLoaded) {
        if (event.isSuccess) {
          emit(state.copyWith(
            globalMessageCount: event.count,
          ));
        }
      }
  ...
  void _getGlobalMessageCount() {
    _server.getGlobalMessagesCount(onSuccess: (count) {
      add(GlobalMessageCountLoaded(
        isSuccess: true,
        count: count,
      ));
    }, onError: (error) {
  ...
  void _resetGlobalMessageCount() {
    _server.resetGlobalMessagesCount(onSuccess: () {
      _getGlobalMessageCount();
    }, onError: (error) {
      if (error.isConnectionError) {
        add(ConnectionErrorEvent(errorMessage: error.message));
  ...
  void _loadGlobalMessages() {
    _server.loadGlobalMessages(onSuccess: (messages) {
      add(GlobalMessagesLoaded(
        isSuccess: true,
        messages: messages,
      ));
    }, onError: (error) {
  ...
  BuiltList<GlobalMessage> _filterMessages(
    BuiltList<GlobalMessage> messages,
  ) {
