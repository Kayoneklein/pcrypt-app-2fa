      ...
      child: Padding(
        padding: _showCheckbox
            ? const EdgeInsets.only(
                left: 8.0, right: 0.0, top: 4.0, bottom: 4.0)
            : const EdgeInsets.only(
                ...
                ? PopupMenuButton<PasswordListItemAction>(
                    ...
                    itemBuilder: (BuildContext context) {
                      ...
                      if (_password.shares[0] != null) {
                        final myTeamIds =
                            _myTeams.map((team) => team.id).toSet();
                        final sharedTeamIds = _password.shares[0]!.toSet();
                        isValidTeamOrMember =
                            myTeamIds.intersection(sharedTeamIds).isNotEmpty;
                      } else {
                        final myMemberIds =
                            _myMembers.map((member) => member.userId).toSet();
      ...
      child: InkWell(
        ...
        child: Padding(
          padding: _additionalPadding
              ? const EdgeInsets.only(
                  left: 56.0, right: 0.0, top: 4.0, bottom: 4.0)
              : const EdgeInsets.only(
                  left: 20.0, right: 0.0, top: 4.0, bottom: 4.0),
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.center,
