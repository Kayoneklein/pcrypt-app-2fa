  ...
  Widget build(BuildContext context) {
    ...
    if (titleController.text != state.locationTitle) {
      titleController.value =
          titleController.value.copyWith(text: state.locationTitle);
    }
    if (latitudeController.text != state.locationLatitude) {
      latitudeController.value =
          latitudeController.value.copyWith(text: state.locationLatitude);
    }
    if (longitudeController.text != state.locationLongitude) {
      longitudeController.value =
          longitudeController.value.copyWith(text: state.locationLongitude);
    }
    if (accuracyController.text != state.locationAccuracy) {
      accuracyController.value =
          accuracyController.value.copyWith(text: state.locationAccuracy);
    }
    return SafeArea(
      child: ClipRRect(
              ...
              children: <Widget>[
                Padding(
                  padding: const EdgeInsets.only(
                      left: 24.0, right: 0.0, top: 24.0, bottom: 0.0),
                  child: Row(
                      ...
                      children: <Widget>[
                        ...
                        if (state.myLocationState == ButtonState.loading)
                          Container(
                            margin: const EdgeInsets.only(
                                top: 12.0, bottom: 12.0, right: 24.0),
                            height: 24.0,
                            width: 24.0,
                        ...
                        if (state.myLocationState == ButtonState.visible)
                          TextButton(
                            ...
                            child: Text(Strings.passwordLocationGetCurrent
                                .toUpperCase()),
                            onPressed: () {
