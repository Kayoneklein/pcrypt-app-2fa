...
import 'package:pcrypt/authentication/bloc/authentication.dart' as auth_bloc;
import 'package:pcrypt/authentication/bloc/authentication_state.dart'
    as auth_state;
...
import 'package:pcrypt/model/sorting_order.dart';
...
class PasswordsBloc extends Bloc<PasswordsEvent, PasswordsState> {
  PasswordsBloc({
    ...
    on<PasswordsEvent>((event, emit) async {
          ...
          if (!event.password.isShared) {
            emit(state.copyWith(
              selectedPasswordIds: _toggleSetItem(
                  state.selectedPasswordIds, event.password.tempId),
            ));
          }
        }
      }
      ...
      if (event is PasswordLongPressed) {
        if (state.selectionModeActive) {
          ...
          emit(state.copyWith(
            ...
            selectedPasswordIds: BuiltSet.from(event.password.isShared
                ? <int>{}
      ...
      if (event is PasswordAddPressed) {
        ...
        // if(await _settings
        //     .getBoolean(Settings.IS_FROM_SAVE_PASSWORD) == true){
        //   var data = await _getpasswordData();
        //   print(data);
        //   event.password.password = jsonDecode(data!)['saveInfo']['password'];
        //   event.password.user = jsonDecode(data)['saveInfo']['username'];
        // }
        event.password.icon = await _loadPasswordIcon(event.password.url);
        final BuiltList<Password> passwords =
            state.myPasswords.rebuild((b) => b.add(event.password));
        final BuiltList<Group> groups =
            _checkForNewGroupsInPassword(event.password, state.allGroups);
        _savePasswords(passwords.toList());
        final filters = _generateFilters(groups, state.sharedPasswords,
            state.myPasswords, state.myTeams, state.myMembers, _configuration);
        emit(state.copyWith(
      ...
      if (event is PasswordUpdated) {
        if (state.myPasswords.any((p) => p.tempId == event.password.tempId)) {
          emit(state.copyWith(isLoading: true));
          event.password.icon = await _loadPasswordIcon(event.password.url);
          final BuiltList<Password> passwords = state.myPasswords.rebuild((b) =>
              b.map((p) =>
                  p.tempId == event.password.tempId ? event.password : p));
          final BuiltList<Group> groups =
              _checkForNewGroupsInPassword(event.password, state.allGroups);
      ...
      if (event is PasswordSharePressed) {
        if (state.myPasswords.any((p) => p.tempId == event.password.tempId)) {
          final password = state.myPasswords
              .firstWhere((p) => p.tempId == event.password.tempId);
          if (_restrictedFunctionality && password.files.isNotEmpty) {
            emit(PremiumRequiredState.from(state));
          } else {
            emit(SharingPermittedState.from(state,
                passwordsToShare: [password]));
          }
        }
      }
      //Password deleted
      ...
      if (event is PasswordShareInfoPressed) {
        emit(state.copyWith(
          currentShareInfo: event.password.shareInfo,
        ));
      }
      //All passwords selected
      if (event is SelectAllPressed) {
        emit(state.copyWith(
          selectedPasswordIds: BuiltSet.from(state.filteredPasswords
              .where((p) => !p.isShared)
              .map<int>((p) => p.tempId)),
        ));
      }
      ...
      if (event is DeselectAllPressed) {
        emit(state.copyWith(
          selectedPasswordIds: BuiltSet.from(<int>[]),
        ));
      }
      ...
      if (event is ShareSelectedPressed) {
        if (state.selectedPasswordIds.isNotEmpty) {
          ...
          if (_restrictedFunctionality &&
            ...
            emit(SharingPermittedState.from(
              state.copyWith(
                selectionModeActive: false,
                selectedPasswordIds: BuiltSet(<int>{}),
              ),
      ...
      if (event is PasswordsSaved) {
        emit(PasswordsSavedState.from(state,
            isSavedSuccessfully: event.isSuccess));
      }
      ...
      if (event is PasswordsShared) {
        _loadPasswords();
        if (event.isSuccess) {
          emit(PasswordsSharedState.from(
            state.copyWith(
      ...
      if (event is SaveOrUpdateConfirmationRequested) {
        ...
        if (event.passwords.isEmpty) {
          ...
          emit(SaveOrUpdateConfirmationRequestedState.from(
            state,
  ...
  Future<void> _storePasswordData(final List<Password>? myPasswords) async {
    try {
      ...
      await methodChannel.invokeMethod<void>(
          'storePasswordData', {'password': jsonEncode(myPasswords)});
  ...
  Future<void> _sendMultiplePassword(List<Password> passwords) async {
    await AutofillService().resultWithDatasets(passwords
        .map((password) => PwDataset(
              label: password.name,
              username: password.user,
              password: password.password,
            ))
  ...
  Future<void> _loadPasswords() async {
    ...
    _server.loadGroups(
      onSuccess: (g) {
        ...
        _server.loadTeamsAndMembers(onSuccess: (teamsList, membersList) {
          _server.loadPasswords(
            onSuccess: (p) async {
              final passwords =
                  _processPasswordsAfterLoading(BuiltList.from(p), groups);

              if (await _settings
                  .getBoolean(Settings.IS_FROM_AUTO_FILL_REQUEST)) {
                  ...
                  await _server.loadSharedPasswords(
                    onSuccess: (sp) async {
                      ...
                      if (!(_currentAuthState is auth_state.BiometricLock) &&
                          ...
                          !(_currentAuthState is auth_state.Unauthenticated)) {
                        add(
                          PasswordFilterByDomain(
                            myDomain: jsonDecode(response ?? '{}')['webDomains']
                                [0]['domain'],
              ...
              } else if (await _settings
                      .getBoolean(Settings.IS_FROM_SAVE_PASSWORD) ==
                  true) {
                await _saveOrUpdatePassword(passwords, teamsList, membersList);
              } else {
                _server.loadSharedPasswords(
                  onSuccess: (sp) {
                    final sharedPasswords =
                        _processSharedPasswordsAfterLoading(BuiltList.from(sp));
                    add(PasswordsLoaded(
                      isSuccess: true,
                      myPasswords: passwords,
                      sharedPasswords: sharedPasswords,
  ...
  Future<void> _saveNewPasswordAndLoad(
    ...
    print('data');
    print(data);
    print('data web domains');
    print(data?['webDomains']);
    print('data save info');
    print(data?['saveInfo']);
    final domains = data?['webDomains'] as List<dynamic>? ?? <dynamic>[];
    final username = data?['saveInfo']['username'] as String? ?? '';
    final String passwordName =
        domains.isEmpty ? username : domains.first['domain'];
        ...
        if (await _settings.getBoolean(Settings.IS_FROM_SAVE_PASSWORD) ==
          ...
          Future.delayed(const Duration(seconds: 0), () async {
            ...
            await _settings.setBoolean(
                Settings.IS_FROM_AUTO_FILL_REQUEST, false);
    ...
    _server.loadSharedPasswords(
      onSuccess: (sp) async {
        ...
        add(PasswordsLoaded(
          ...
          myMembers:
              BuiltList.from(membersListForSaveOrUpdate ?? <TeamMember>[]),
        ));
      ...
      onSuccess: (passwords) async {
        final newPasswords = state.myPasswords.rebuild((b) {
          for (var password in passwords) {
            final int index = state.myPasswords
                .indexWhere((p) => p.tempId == password.tempId);
            if (index >= 0) {
              b.removeAt(index);
  ...
  Future<void> _deleteShares(
      List<Password> passwordsToDelete, BuiltList<Password> myPasswords) async {
    await _server.deleteSharedPasswords(
      passwordsToDelete: passwordsToDelete,
      myPasswords: myPasswords.toList(),
      onSuccess: (passwords) async {
        final newPasswords = myPasswords.rebuild((b) => b.removeWhere(
    ...
    if (location != null) {
      final passwords = _applyFilter(
        ...
        state.searchQuery,
          ...
          .where((p) => p.locations
              .any((l) => _locations.testLocationsOverlap(location, l)))
          .toList();
      add(LocationFilterApplied(
        isSuccess: true,
        passwords: passwords,
      ));
    } else {
  ...
  Map<_FilterType, List<Filter>> _generateFilters(
    ...
    final mySharedPasswordIds =
        myPasswords.expand<int>((p) => p.shares[0] ?? []).toSet();
    final filteredMyTeamIds = myTeamIds.intersection(mySharedPasswordIds);

    final myTeamsDictionary = Map<int, String>.fromIterable(myTeams,
        key: (dynamic item) => item.id, value: (dynamic item) => item.name);
    ...
    final myMembersDictionary = Map<int, TeamMember>.fromIterable(myMembers,
        key: (dynamic item) => item.userId, value: (dynamic item) => item);
    ...
    final sharedCreatorFilters =
        sharedPasswords.map((p) => p.creator).whereType<TeamMember>().toList();
  ...
  BuiltList<Group> _checkForNewGroupsInPassword(
      Password password, BuiltList<Group> currentGroups) {
    return currentGroups.rebuild((b) => b.addAll(
  ...
  BuiltList<Password> _processPasswordsAfterLoading(
      BuiltList<Password> passwords, BuiltList<Group> groups) {
    return BuiltList.from(passwords.asMap().map<int, Password>((i, p) {
      p.tempId = i + 1;
      p.tags = p.groupIds
          .map<Group?>((id) {
  ...
  BuiltList<Password> _applyFilter(
    ...
    final ListBuilder<Password> builder =
        (myPasswords + sharedPasswords).toBuilder();
      ...
      case PasswordSortingOrder.tagAZ:
        return (Password first, Password second) {
          final String firstName =
              first.tags.isNotEmpty ? first.tags.first.name.toLowerCase() : '';
