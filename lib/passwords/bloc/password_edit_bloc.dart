...
class PasswordEditBloc extends Bloc<PasswordEditEvent, PasswordEditState> {
    ...
    on<PasswordEditEvent>((event, emit) async {
      ...
      if (event is GroupsLoadingFinished) {
        if (event.groups != null) {
          ...
          emit(state.copyWith(
            groupsLoadingStatus: LoadingStatus.error,
          ));
        }
      }
      ...
      if (event is UsernameChanged) {
        emit(state.copyWith(
          ...
          isUsernameValid:
              (event.username != state.username) ? true : state.isUsernameValid,
        ));
      }
      ...
      if (event is UrlChanged) {
        emit(state.copyWith(
          url: event.url,
          isUrlValid: _isUrlValid(event.url),
        ));
      }
      ...
      if (event is SelectTagsPressed) {
        emit(state.copyWith(
          ...
          selectedGroupIds: BuiltSet.from(state.tags.map<String>((t) => t.id)),
        ));
      }
      if (event is TagSelectionChanged) {
        emit(state.copyWith(
          selectedGroupIds: _toggleSetItem(state.selectedGroupIds, event.tagId),
        ));
      }
      if (event is TagsSelectionConfirmed) {
        if (state.groupsLoadingStatus == LoadingStatus.loaded) {
          emit(state.copyWith(
              ...
              tags: BuiltList<Group>.from(state.allGroups
                  .where((g) => state.selectedGroupIds.contains(g.id)))));
        }
      }
      ...
      if (event is LocationAddressInputConfirmed) {
        emit(state.copyWith(
          getAddressOkState: ButtonState.loading,
        ));
        final location =
            await _server.getLocationFromAddress(state.locationAddress);
        if (location != null) {
          add(GetLocationFromAddressFinished(
            isSuccess: true,
            location: location,
          ));
        } else {
      ...
      if (event is GetLocationFromAddressFinished) {
        if (event.isSuccess && event.location != null) {
          ...
          emit(state.copyWith(
            ...
            locationTitle: location.title,
            locationLatitude: location.latitude.toString(),
            isLocationLatitudeValid: _isLatValid(location.latitude.toString()),
            locationLongitude: location.longitude.toString(),
            isLocationLongitudeValid:
                _isLonValid(location.longitude.toString()),
            locationAccuracy: location.accuracy.toString(),
            isLocationAccuracyValid: _isAccValid(location.accuracy.toString()),
      ...
      if (event is GetCurrentLocationPressed) {
        emit(state.copyWith(
          myLocationState: ButtonState.loading,
        ));
      ...
      if (event is LocationDeletePressed) {
        emit(state.copyWith(
          locations: state.locations.rebuild((b) => b.removeAt(event.position)),
        ));
      }
      ...
      if (event is FileUploadFinished) {
        if (event.result == FileUploadResult.largerThan1Mb) {
          ...
          emit(FileUploadResultState.from(
            state,
    ...
    await permissionServices().then((value) {
      if (value[Permission.location]?.isGranted == true) {
        _locations.determineCurrentLocation(
          onSuccess: (location) {
            add(GetCurrentLocationFinished(
              isSuccess: true,
              location: location,
            ));
          },
  ...
  Future<void> _uploadFile(String path) async {
    try {
      ...
      _server.saveFile(
        ...
        data: data,
        onSuccess: () {
          add(FileUploadFinished(
            result: FileUploadResult.success,
            file:
                Attachment(fileId, basename(path), lookupMimeType(path) ?? ''),
          ));
        },
        onError: (error) {
          if (error.isSessionExpired) {
  ...
  Password _generatePassword() => Password(
        ...
        shareChanges: (_originalPassword?.shareChanges ?? {})
          ..addAll(_originalPassword?.shares.map<int, List<int>>(
