...
import 'package:pcrypt/config/configuration_event.dart';
import 'package:pcrypt/constants/preferences.dart';
import 'package:pcrypt/login/bloc/login_settings.dart';
import 'package:pcrypt/settings/bloc/settings.dart' as settings;
...
class _LoginSettingsFormState extends State<LoginSettingsForm> {
  ...
  Future<void> _initPreferences() async {
    final String? _customServerUrl = await Preferences().customServerUrl;
    final _isCustomServer = await Preferences().isCustomServer;

    if (_customServerUrl != null && _customServerUrl.isNotEmpty == true) {
      _bloc.add(
        CustomServerUrlChanged(url: _customServerUrl),
      );
    }

    _bloc.add(
      ServerSelectionChanged(isCustomServer: _isCustomServer),
    );
  }
  ...
  Future<void> _stateChangeListener(
      BuildContext context, LoginSettingsState state) async {
    if (state.customServerUrl != _serverController.text) {
      _serverController.text = state.customServerUrl;
    }
