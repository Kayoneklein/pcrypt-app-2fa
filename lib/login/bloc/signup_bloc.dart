    ...
    await _server.signUp(
      ...
      onSuccess: (user) async {
        await _server.saveGroups(
          groups:
              jsonDecode(l10n('default', 'PCDEFAULTGROUPJSON')).map<Group>((dynamic j) => Group.fromJson(j)).toList(),
        );
        final Map<String, dynamic> team = jsonDecode(l10n('default', 'PCDEFAULTTEAMJSON'));
        ...
        add(const SignUpResultReceived(
      ...
      onError: (error) {
        if (error.isWrongParameter) {
          ...
          add(const SignUpResultReceived(
            status: SignUpStatus.accountExists,
          ));
