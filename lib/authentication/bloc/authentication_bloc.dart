...
import 'package:pcrypt/util/app_reset.dart';
...
class AuthenticationBloc
    ...
    try {
      on<AuthenticationEvent>((event, emit) async {
        ...
        if (event is SignedInEvent) {
          ...
          if (event.minutesTillAutoLogout > 0) {
            await Future.delayed(Duration(minutes: event.minutesTillAutoLogout),
                () {
              // print('event.minutesTillAutoLogout Inside the timer function');
              // print(event.minutesTillAutoLogout);
              emit(SessionExpired());
            });
          }
        }
        ...
        if (event is SignedOutEvent) {
          // await _settings.setBoolean(Settings.LOGIN_BIOMETRICS_CHECKBOX, false);
          _storePasswordData();
          _stopTimer();
          // _saveMinutesTillLogout(null);
          // _saveBiometricsEnabled(null);
          await _settings.setLoggedOut();
          await _server.clearSessionData();
          emit(Unauthenticated());
        }
  ...
  Future<void> _storePasswordData() async {
    try {
      await methodChannel.invokeMethod<void>(
          'storePasswordData', {'password': jsonEncode('[]')});
  ...
  Future<void> _saveMinutesTillLogout(int? minutes) async {
    if (minutes != null) {
      await _settings.setString(
          Settings.LOGIN_SESSION_DURATION, minutes.toString());
    } else {
  ...
  Future<int> _loadMinutesTillLogout() async {
    final String? min =
        await _settings.getString(Settings.LOGIN_SESSION_DURATION);
  ...
  Future<void> _saveLastActiveTime() async {
    await _settings.setString(Settings.LOGIN_SESSION_LAST_ACTIVE,
        DateTime.now().millisecondsSinceEpoch.toString());
  }

  Future<DateTime?> _loadLastActiveTime() async {
    final String? ms =
        await _settings.getString(Settings.LOGIN_SESSION_LAST_ACTIVE);
