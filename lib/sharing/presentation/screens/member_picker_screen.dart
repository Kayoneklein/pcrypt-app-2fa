...
import 'package:pcrypt/sharing/bloc/member_picker.dart';
import 'package:pcrypt/sharing/presentation/widgets/email_list.dart';
import 'package:pcrypt/sharing/presentation/widgets/members_list.dart';
import 'package:pcrypt/sharing/presentation/widgets/team_picker.dart';
import 'package:pcrypt/sharing/presentation/widgets/teams_list.dart';
        ...
        if (_emailController.text != state.email) {
          if (state.email.isEmpty) {
            ...
            _emailController.value =
                _emailController.value.copyWith(text: state.email);
          }
        }
        return Scaffold(
    ...
    if (state is MembersProcessedState) {
      if (state.isProcessingSuccess &&
        ...
        if (state.unprocessedEmails!.isEmpty) {
          Navigator.of(context).pop<MemberPickerModel>(MemberPickerModel(
            initialMembersToSend: [],
            membersToSend: state.processedMembers!,
          ));
        } else {
          showUnprocessedWarningDialog(
              state.processedMembers!, state.unprocessedEmails!);
        }
      } else {
        ScaffoldMessenger.of(context)
          ..hideCurrentSnackBar()
          ..showSnackBar(SnackBar(
            content: Text(Strings.memberPickerEmailProcessingError),
          ));
      }
    }
  }
    ...
    await showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(Strings.memberPickerEmailUnprocessedMessage +
              '\n\n' +
