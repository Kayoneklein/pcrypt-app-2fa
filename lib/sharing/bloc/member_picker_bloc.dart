  ...
  MemberPickerBloc({
    ...
    on<MemberPickerEvent>((event, emit) {
      ...
      if (event is MembersLoaded) {
        if (event.isSuccess) {
          final emailTeams =
              BuiltList<Team>.from(event.teams!.where((t) => t.isAdmin));
          ...
          emit(state.copyWith(
            ...
            allTeams: allTeams,
            allMembers: BuiltList<TeamMember>.from(event.members!
                .where((m) =>
                    m.userId > -1 && m.teamId > -1 && m.publicKey != null)
                .toList()),
            emailTeams: emailTeams,
            selectedEmailTeamId:
                emailTeams.isNotEmpty ? emailTeams.first.id : -1,
      ...
      if (event is TeamSelectionChanged) {
        emit(state.copyWith(
          ...
          filteredMembers:
              _filterMembers(state.allMembers, event.teamId, state.searchQuery),
        ));
      }
        ...
        if (state.currentTab == MemberPickerTab.members) {
          emit(state.copyWith(
            ...
            filteredMembers: _filterMembers(
                state.allMembers, state.selectedTeamId, event.text),
          ));
        }
        if (state.currentTab == MemberPickerTab.teams) {
          emit(state.copyWith(
        ...
        if (state.emailTeams != null) {
          if (state.email.isNotEmpty &&
            ...
            if (state.emailList.any((m) =>
              ...
              emit(state.copyWith(
                emailList:
                    state.emailList.rebuild((b) => b.add(TeamMember.email(
                          email: state.email,
      ...
      if (event is EmailItemDeletePressed) {
        emit(state.copyWith(
          emailList: state.emailList.rebuild((b) => b.removeWhere((m) =>
          ...
          case MemberPickerTab.members:
            if (state.selectedMemberIds != state.initialSelectedMemberIds ||
              ...
              List<TeamMember> generateMembers(
                ...
                return pair
                    .map<TeamMember?>((id) {
                      final findings = state.allMembers.where((m) =>
                          m.userId == id.memberId && m.teamId == id.teamId);
              ...
              final initialSelectedMembers =
                  generateMembers(state.initialSelectedMemberIds);
              ...
              final List<TeamMember> selectedTeams =
                  generateTeams(state.selectedTeamIds);
              final List<TeamMember> initialSelectedTeams =
                  generateTeams(state.initialSelectedTeamIds);
              ...
              emit(SelectionConfirmedState.from(
                state,
      ...
      if (event is EmailsFinishedProcessing) {
        emit(MembersProcessedState.from(
...
*/
  ...
  BuiltList<TeamMember> _filterMembers(
      BuiltList<TeamMember> members, int selectedTeamId, String searchQuery) {
