...
import 'package:pcrypt/model/location.dart' as pc;
...
class LocationService {
  ...
  Future<pc.Location?> getLocationFromAddress(String address) async {
    try {
      ...
      if (data.isNotEmpty) {
        ...
        final title =
            placemarks.isNotEmpty ? formatAddress(placemarks.first) : '';
        return pc.Location(
  ...
  Future<void> determineCurrentLocation({
    ...
    final bool serviceEnabled =
        await location_service.Location().serviceEnabled();
    if (serviceEnabled) {
      try {
      ...
      final bool isUserEnableService =
          await location_service.Location().serviceEnabled();
  ...
  Future<bool> checkLocationServicesEnabled() async {
    ...
    if (status != LocationPermission.deniedForever) {
      final bool isLocationDenied =
          await _settings.getBoolean(Settings.IS_LOCATION_DENIED);
      if (isLocationDenied) {
  ...
  bool testLocationsOverlap(pc.Location first, pc.Location second) {
    ...
    final accuracy = (first.accuracy <= 0 ? 100.0 : first.accuracy) +
