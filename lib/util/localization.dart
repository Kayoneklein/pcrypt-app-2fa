...
String l10n(String section, String key) => Localization.get.localize(section, key);
...
class Localization {
  ...
  Future<Language> _getLocalLanguage() async {
    try {
      return Language.fromJson(jsonDecode(await _storage.read(key: 'l10n_current') ?? ''));
    } catch (error) {
  ...
  Future<void> _setLocalLanguage(Language language) async {
    if (language != Language.undefined) {
      await _storage.write(key: 'l10n_current', value: jsonEncode(language.toJson()));
    } else {
  ...
  Future<List<Language>> _getLocalLanguages() async {
    try {
      return List<Language>.from(
        jsonDecode(await _storage.read(key: 'l10n_languages') ?? '').map((dynamic e) => Language.fromJson(e)),
      );
    } catch (error) {
      return [Language.def];
    }
  ...
  Future<_LocalizationData> _getLocalData() async {
    try {
      return _LocalizationData.fromJson(jsonDecode(await _storage.read(key: 'l10n_data') ?? ''));
    } catch (error) {
      return _LocalizationData.empty;
    }
  }
...
class _LocalizationData {
  ...
  static _LocalizationData fromJson(Map<String, dynamic> json) => _LocalizationData._(
        json.map((k, dynamic v) => MapEntry<String, Map<String, String>>(k, Map<String, String>.from(v))),
      );
