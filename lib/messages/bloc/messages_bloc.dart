...
class MessagesBloc extends Bloc<MessagesEvent, MessagesState> {
    ...
    on<MessagesEvent>((event, emit) {
      ...
      if (event is CurrentBoxChanged) {
        emit(state.copyWith(
            ...
            searchQuery: '',
            displayedMessages: _filterMessages(event.box, state.inboxMessages,
                state.outboxMessages, state.currentSortingOrder, '')));
      }
      ...
      if (event is MessagesLoaded) {
        if (event.isSuccess) {
          ...
          emit(state.copyWith(
            ...
            displayedMessages: _filterMessages(state.currentBox, inbox, outbox,
                state.currentSortingOrder, state.searchQuery),
          ));
        } else {
          emit(state.copyWith(
      ...
      if (event is MessagePressed) {
        if (state.selectionModeActive) {
          emit(state.copyWith(
            selectedMessageIds:
                _toggleSetItem(state.selectedMessageIds, event.message.mailId),
          ));
        }
      }
      ...
      if (event is SelectAllPressed) {
        emit(state.copyWith(
          selectedMessageIds:
              BuiltSet.from(state.displayedMessages.map<int>((m) => m.mailId)),
        ));
      }
      ...
      if (event is MessageSentSuccess) {
        final outbox =
            state.outboxMessages.rebuild((b) => b.addAll(event.messages));
        emit(MessageSentState.from(
          state.copyWith(
            outboxMessages: outbox,
            displayedMessages: _filterMessages(
                state.currentBox,
      ...
      if (event is DeleteSelectedPressed) {
        _deleteMessages(state.displayedMessages
            .where((m) => state.selectedMessageIds.contains(m.mailId))
            .toList());
        switch (state.currentBox) {
          case MessagesBox.inbox:
            final inbox = state.inboxMessages.rebuild((b) => b.removeWhere(
                (m) => state.selectedMessageIds.contains(m.mailId)));
            emit(state.copyWith(
              inboxMessages: inbox,
              displayedMessages: _filterMessages(state.currentBox, inbox,
                  state.outboxMessages, state.currentSortingOrder, ''),
              selectionModeActive: false,
              selectedMessageIds: BuiltSet.from(<int>{}),
              isLoading: true,
          ...
          case MessagesBox.outbox:
            final outbox = state.outboxMessages.rebuild((b) => b.removeWhere(
                (m) => state.selectedMessageIds.contains(m.mailId)));
            emit(state.copyWith(
              outboxMessages: outbox,
              displayedMessages: _filterMessages(state.currentBox,
                  state.inboxMessages, outbox, state.currentSortingOrder, ''),
              selectionModeActive: false,
              selectedMessageIds: BuiltSet.from(<int>{}),
              isLoading: true,
            ));
      ...
      if (event is MessagesDeleted) {
        emit(MessagesDeletedState.from(
          state.copyWith(isLoading: false),
  ...
  void _loadMessages() {
    _server.loadMessages(onSuccess: (inbox, outbox) {
      add(MessagesLoaded(
        isSuccess: true,
        inbox: inbox,
        outbox: outbox,
