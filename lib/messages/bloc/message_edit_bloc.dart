...
class MessageEditBloc extends Bloc<MessageEditEvent, MessageEditState> {
  ...
  }) : super(MessageEditState.initial(
          subject: subject ?? '',
          message: message ?? '',
          members: members ?? [],
        )) {
    on<MessageEditEvent>((event, emit) async {
      ...
      if (event is SubjectChanged) {
        emit(state.copyWith(
          ...
          isSubjectValid:
              (event.subject != state.subject) ? true : state.isSubjectValid,
        ));
      }
  ...
  MessageInfo _generateMessage() {
    final messageInfo =
        MessageInfo(subject: state.subject, message: state.message);
