...
import 'package:pcrypt/messages/bloc/message_edit.dart';
...
class MessageEditScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<MessageEditBloc>(
      ...
      child: MessageEditForm(
        isNew: message?.isEmpty == true,
      ),
    );
  }
}
...
class _MessageEditFormState extends State<MessageEditForm> {
      ...
      child: BlocBuilder<MessageEditBloc, MessageEditState>(
        ...
        if (_subjectController.text != state.subject) {
          _subjectController.value =
              _subjectController.value.copyWith(text: state.subject);
        }
        if (_messageController.text != state.message) {
          _messageController.value =
              _messageController.value.copyWith(text: state.message);
        }
        return PopScope(
          // canPop: false,
  ...
  Future<void> _blocCommandListener(
      BuildContext context, MessageEditState state) async {
