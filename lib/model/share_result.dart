...
class Share1Result {
  ...
  static Share1Result fromJson(Map<String, dynamic> data) => Share1Result(
        shareData: data['sharedata'].map<int, List<PasswordShare>>((String k, dynamic v) {
...
class Share2ResultV2 {
  ...
  static Share2ResultV2 fromJson(Map<String, dynamic> data) {
    ...
    if (data['user'] != null) {
      ...
      result.noDelUserSource = data['user']['nodelsource'].map<int, List<String>>((String k, dynamic v) {
    ...
    if (data['team'] != null) {
      result.newTeamFiles = List<TeamFileShare>.from(data['team']['newfiles']
          .map<List<TeamFileShare>>((dynamic e) {
            final result = <TeamFileShare>[];
            for (int i = 0; i < e['fileids'].length; i++) {
              result.add(TeamFileShare(
                index: e['index'],
                fileId: e['fileids'][i],
                teamId: e['teamids'][i],
                keyId: e['keyids'][i],
      ...
      result.noDelTeamSource = data['team']['nodelsource'].map<int, List<String>>((String k, dynamic v) {
        return MapEntry<int, List<String>>(
          int.parse(k),
          v.map<String>((dynamic e) => e.toString()).toList(),
        );
